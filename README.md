...
### プロジェクト概要 (Application Overview)

本アプリケーションは、**「resale-app」**という名称のフリマアプリケーションです。  

ユーザーは、会員登録・ログイン後に商品の**出品、購入、および「いいね」や「コメント」**といった交流機能を利用できます。  

#### 主な機能

* **商品一覧/検索**: 全商品表示、購入済み商品への "Sold" 表示、商品名による部分一致検索。
* **商品詳細**: 画像、名前、価格、カテゴリ（複数対応）、コメント投稿/閲覧、いいね機能。
* **購入**: 配送先住所の変更、支払い方法の選択（表示のみ）、購入後の商品状態更新。
* **出品**: ローカルからの画像アップロード、複数カテゴリの登録。
* **マイページ**: プロフィール編集（画像含む）、出品した商品一覧、購入した商品一覧の確認。
...
- 成功メッセージ表示
  - 商品一覧画面表示：「コメント投稿」「商品出品処理」「プロフィール設定」「商品購入処理」
  - 商品購入画面表示：「配送先住所更新」
